...
/**
 * Unit tests for [UvRegionPacker.pack] — the composed affine matrix written into
 * the `sceneUvRegions` GPU buffer (one `mat3x2<f32>` = 24 bytes per face).
 *
 * Column-major layout read order:
 *   floats[0..1] = col0 (u-axis),  floats[2..3] = col1 (v-axis),  floats[4..5] = col2 (translation)
 *
...
class GpuTextureManagerUvTransformTest {
    ...
    private fun assertFloats(
        ...
        val names = arrayOf("col0.x", "col0.y", "col1.x", "col1.y", "col2.x", "col2.y")
     ...
     * Expected: diagonal scale + atlas offset, zeroes on off-diagonals.
     *
     ...
     */
    @Test
    fun `identity writes atlas-only diagonal matrix`() {
        val buf = makeBuffer()
        UvRegionPacker.pack(
            buf          = buf,
            atlasScaleU  = 0.5f,
        ...
        assertFloats(floatArrayOf(0.5f, 0f, 0f, 0.5f, 0.1f, 0.2f), readFloats(buf))
    }
    ...
    fun `tiling 2x3 scales matrix correctly`() {
        ...
        assertFloats(floatArrayOf(2f, 0f, 0f, 3f, -0.5f, -1.0f), readFloats(buf))
    }
    ...
    fun `offset shifts translation column`() {
        ...
        assertFloats(floatArrayOf(1f, 0f, 0f, 1f, 0.5f, 0f), readFloats(buf))
    }
