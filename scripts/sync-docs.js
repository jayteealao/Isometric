...
// ── Copy doc screenshots to site public directory ─────────────────────────────
// Keeps docs/assets/screenshots/ (generated by tests) in sync with
// site/public/screenshots/ so the Starlight site can reference them as
// /screenshots/foo.png without a build-time copy step.
function copyScreenshots() {
  const screenshotSrc = join(ROOT, 'docs', 'assets', 'screenshots');
  const screenshotDst = join(ROOT, 'site', 'public', 'screenshots');
  if (!existsSync(screenshotSrc)) return;
  mkdirSync(screenshotDst, { recursive: true });
  cpSync(screenshotSrc, screenshotDst, { recursive: true });
  console.log(`  screenshots  →  site/public/screenshots/`);
}
