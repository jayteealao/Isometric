@file:OptIn(io.fabianterhorst.isometric.ExperimentalIsometricApi::class)
...
import io.fabianterhorst.isometric.compose.runtime.Path as IsoPath
import io.fabianterhorst.isometric.compose.runtime.Group
        ...
        paparazzi.snapshot {
            Box(modifier = Modifier.size(680.dp, 440.dp)) {
                IsometricScene {
                    Shape(geometry = Prism(Point.ORIGIN, 3.0, 3.0, 1.0), color = BLUE)
                    Shape(
                        geometry = io.fabianterhorst.isometric.Shape.extrude(
                            Path(
                                listOf(
                                    Point(1.0, 1.0, 1.0),
                                    Point(2.0, 1.0, 1.0),
