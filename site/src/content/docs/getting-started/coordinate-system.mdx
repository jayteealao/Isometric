...
The general formula is `x + y - z / sin(angle)`. At the default 30°, `sin(30°) = 0.5`, so `z / sin(30°) = 2z` and the formula reduces exactly to `x+y−2z`. The engine threads its configured angle through the sort so ordering stays correct at any angle. See [Depth Sorting](/concepts/depth-sorting/) for the full algorithm.
