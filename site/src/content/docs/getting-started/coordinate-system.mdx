...
For non-default projection angles the metric generalizes to `x · cos(angle) + y · sin(angle) − 2 · z`, and the engine threads its configured angle through the sort so ordering stays correct. See [Depth Sorting](/concepts/depth-sorting/) for the full algorithm.
