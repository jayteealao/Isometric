...
[Tile Grid](/guides/tile-grid/) for the high-level routing.
...
See [Scene Configuration](/reference/scene-config/) for the full `AdvancedSceneConfig` table and
[Per-Node Interactions](/guides/interactions/) for the higher-level `onClick` path.
