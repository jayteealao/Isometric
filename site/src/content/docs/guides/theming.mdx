...
[Per-Node Interactions](/guides/interactions/#alpha).
...
[CompositionLocals reference](/reference/composition-locals/) for the full table of
...
`StrokeStyle` is a sealed class with three variants: `FillOnly`, `Stroke(width, color)`, and `FillAndStroke(width, color)` (default). See [Scene Config reference](/reference/scene-config/) for the full variant table.
