...
All Isometric locals use `staticCompositionLocalOf` rather than `compositionLocalOf`. See [Scene Graph — Compose Runtime Integration](/concepts/scene-graph/#the-compose-runtime-integration) for why this is the correct trade-off.
