...
See [Rendering Pipeline](/concepts/rendering-pipeline/) for how these flow through
projection, caching, and drawing.

## Direct Usage Outside Compose

`IsometricEngine` can be used as a standalone projection library without the Compose runtime, for server-side rendering, screenshot generation, or non-Compose UI frameworks. See [Advanced Patterns — Direct Engine Usage](/examples/advanced-patterns/#direct-engine-usage-outside-compose) for a full worked example.
