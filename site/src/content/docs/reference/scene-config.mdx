...
Use `AdvancedSceneConfig` when you need lifecycle hooks, performance tuning beyond `RenderOptions`, or a custom engine. For most scenes, `SceneConfig` is sufficient.
...
IsometricScene(
    config = AdvancedSceneConfig(
        ...
        onRenderError = { commandId, error ->
            Log.e("Isometric", "Render error in $commandId", error)
        },
        onAfterDraw = {
            // Draw a debug overlay after the scene
