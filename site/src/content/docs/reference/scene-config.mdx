...
| nodeDragState | NodeDragState? | null | State object for the single-node tap-to-select-then-drag affordance. Create with [`rememberNodeDragState`](/reference/composables/#remembernodedragstate); see the [Drag & Camera how-to](/guides/drag-and-camera/). |
...
(`onDoubleClick`). See [Per-Node Interactions](/guides/interactions/).
...
For guidance on when to use `AdvancedSceneConfig` vs `SceneConfig`, see the [Advanced Config guide](/guides/advanced-config/).
