...
For non-default angles the metric generalizes to `x * cos(angle) + y * sin(angle) - 2 * z`. The engine threads its configured projection angle through the sort, so a scene rendered at a non-30° angle still orders correctly. See [Coordinate System](/getting-started/coordinate-system/) for the full projection math.
