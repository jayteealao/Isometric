...
The cell size trades cell count against candidates per cell — a knob worth turning only when shape density is unusual in either direction. For the tuning table and configuration examples, see [Performance — Tuning broadPhaseCellSize](/guides/performance/#tuning-broadphasecellsize).
...
[Performance — Disabling depth sorting or the broad phase](/guides/performance/#disabling-depth-sorting-or-the-broad-phase).
