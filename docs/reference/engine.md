...
`HitOrder` selects which face wins when several overlap at the tap point:

- `HitOrder.FRONT_TO_BACK` (default) — returns the front-most hit, i.e. the face nearest the viewer (drawn on top). This is what you usually want for selection.
- `HitOrder.BACK_TO_FRONT` — returns the back-most hit.

`touchRadius` widens the test by a pixel radius for forgiving taps (`0.0` = exact point-in-polygon).
...
`IsometricEngine` can be used as a standalone projection library without the Compose runtime, for server-side rendering, screenshot generation, or non-Compose UI frameworks. See [Advanced Patterns — Direct Engine Usage](../examples/advanced-patterns.md#direct-engine-usage-outside-compose) for a full worked example.
