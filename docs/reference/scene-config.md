...
| spatialIndexCellSize | Double | 100.0 (`IsometricRenderer.DEFAULT_SPATIAL_INDEX_CELL_SIZE`) | Grid cell size for the hit-test spatial index, in world units. Must be positive and finite. |
| forceRebuild | Boolean | false | Clears the PreparedScene cache before every frame while `true` (disables caching) |
| frameVersion | Long | 0L | Redraw trigger read by the draw phase; bumping redraws the cached scene without re-projection |
| onFlagsReady | ((RuntimeFlagSnapshot) -> Unit)? | null | Receives active runtime flags after config is applied |
| onRenderError | ((commandId: String, error: Throwable) -> Unit)? | null | Called when a render command fails |
| onHitTestReady | ((hitTest: (x: Double, y: Double) -> IsometricNode?) -> Unit)? | null | Receive hit-test function |
| onEngineReady | ((SceneProjector) -> Unit)? | null | Receive engine reference |
| onRendererReady | ((IsometricRenderer) -> Unit)? | null | Receive renderer reference |
