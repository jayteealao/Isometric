...
- **X** — runs right-and-up on screen (+x moves right and upward).
- **Y** — runs left-and-up on screen (+y moves left and upward).
...
For non-default projection angles the metric generalizes to `(x + y) · sin(angle) − 2 · z`. At the default 30°, `sin(30°) = 0.5`, so this reduces to `(x+y) · 0.5 − 2z`, which has the same sort ordering as the simpler `x+y−2z`. The engine threads its configured angle through the sort so ordering stays correct at any angle. See [Depth Sorting](../concepts/depth-sorting.md) for the full algorithm.
...
- **x** — increases right-and-up on screen (same as world X).
- **y** — increases left-and-up on screen (same as world Y).
