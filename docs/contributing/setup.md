...
4. If prompted, install the Kotlin plugin matching version 1.9.22.
...
This starts a local dev server at `http://localhost:4321/Isometric/` (the site is served under the `/Isometric` base path). Pages are MDX files in `site/src/content/docs/`. See the [Documentation Guide](docs-guide.md) for details on writing docs.
