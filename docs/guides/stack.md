...
at runtime. Use a negative value to reverse direction instead of zero. `count` must be at
least 1 — zero or negative counts also throw `IllegalArgumentException`.
...
## Interacting with Stacked Shapes

`Stack` positions its children; the children themselves stay fully interactive. Attach
`onClick`, `onLongClick`, or `onDoubleClick` to the shapes you render inside the stack, and
use the index to identify which layer was tapped. To fade a whole stack, wrap it in a
`Group` with an `alpha` — the group's alpha multiplies into every stacked child (see
[Per-Node Interactions — Alpha on groups](interactions.md#alpha-on-groups)):

```kotlin
Group(alpha = if (highlighted) 1f else 0.4f) {
    Stack(count = 5, axis = StackAxis.Z, gap = 1.0) { floor ->
        Shape(
            geometry = Prism(Point.ORIGIN),
...
- [Per-Node Interactions](interactions.md) — click handlers, alpha, and ids on stacked shapes
