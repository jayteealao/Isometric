...
See [Scene Config reference](../reference/scene-config.md) for the full parameter table.

### Tuning broadPhaseCellSize

`broadPhaseCellSize` controls the grid granularity of the depth sort's broad phase
(see [Depth Sorting](../concepts/depth-sorting.md) for how the pipeline works):
...
See [Rendering Pipeline — Cache Invalidation](../concepts/rendering-pipeline.md#cache-invalidation) for exactly which conditions invalidate the cache.
